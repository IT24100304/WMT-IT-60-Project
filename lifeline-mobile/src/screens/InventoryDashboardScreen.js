...
import { AlertCircle, AlertTriangle, CheckCircle, Package, Search, ChevronDown, ChevronUp, FileText, ExternalLink } from 'lucide-react-native';
...
export default function InventoryDashboardScreen({ navigation }) {
    ...
    const [inventoryDetailsById, setInventoryDetailsById] = useState({});
    ...
    const getAttachmentUrl = (attachment = {}) => {
        const fileUrl = attachment.fileUrl || attachment.filename || '';
        if (!fileUrl) return '';
        if (String(fileUrl).startsWith('http')) return encodeURI(fileUrl);
        return encodeURI(`${api.defaults.baseURL}${fileUrl}`);
    };
    ...
    const getAttachmentTypeLabel = (attachment = {}) => {
        ...
        if (mimeType.startsWith('image/') || /\.(jpg|jpeg|png|webp)$/.test(name)) return 'Image';
        return 'File';
    };

    const toggleInventoryDetails = async (itemId) => {
        const nextExpanded = !expandedInventory[itemId];
        setExpandedInventory(prev => ({ ...prev, [itemId]: nextExpanded }));

        if (!nextExpanded || inventoryDetailsById[itemId]) return;

        setDetailsLoadingId(itemId);
        try {
            const res = await api.get(`/api/inventory/${itemId}/details`);
            setInventoryDetailsById(prev => ({ ...prev, [itemId]: res.data }));
        } catch (error) {
            Alert.alert('Details Error', 'Unable to load full lab details for this blood unit.');
            setExpandedInventory(prev => ({ ...prev, [itemId]: false }));
        } finally {
    ...
    const openAttachment = async (attachment) => {
        ...
        if (!url) {
            Alert.alert('File Error', 'No file URL found for this attachment.');
            return;
        }
        ...
        try {
            ...
            if (supported) {
                ...
                Alert.alert('File Error', 'Unable to open this attachment on this device.');
            }
        } catch (error) {
            Alert.alert('File Error', 'Unable to open this attachment. Please check the file URL.');
        }
    };

    const renderLabDetails = (item) => {
        const details = inventoryDetailsById[item.id];
        const labResults = details?.allLabResults || [];

        if (detailsLoadingId === item.id) {
            return (
                <View style={styles.labDetailsPanel}>
        ...
        return (
            <View style={styles.labDetailsPanel}>
                <Text style={styles.detailsTitle}>Full Lab Details</Text>
                <Text style={styles.detailsLine}>Collected: {details.collectedAt ? new Date(details.collectedAt).toLocaleString() : 'N/A'}</Text>
                <Text style={styles.detailsLine}>Current safety: {details.safetyFlag || 'PENDING'} | Test: {details.testStatus || 'PENDING'}</Text>

                {labResults.length === 0 ? (
                    <Text style={styles.detailsEmpty}>No lab results recorded yet.</Text>
                ) : labResults.map((result, index) => (
                    <View key={`${item.id}-lab-${index}`} style={styles.resultBox}>
                        <Text style={styles.resultTitle}>
                            Test {index + 1}: <Text style={{ color: result.overallResult === 'TESTED_SAFE' ? '#065F46' : '#991B1B' }}>{result.overallResult}</Text>
                        </Text>
                        <Text style={styles.detailsLine}>Tested: {result.testedAt ? new Date(result.testedAt).toLocaleString() : 'N/A'}</Text>
                        <Text style={styles.detailsLine}>Markers: {result.positiveMarkers?.length ? result.positiveMarkers.join(', ') : 'None found'}</Text>
                        {!!result.reason && <Text style={styles.detailsNote}>Reason: {result.reason}</Text>}
                        {!!result.positiveDetails?.notes && <Text style={styles.detailsNote}>Positive notes: {result.positiveDetails.notes}</Text>}
                        {!!result.positiveDetails?.severity && result.positiveMarkers?.length > 0 && (
                            <Text style={styles.detailsLine}>Severity: {result.positiveDetails.severity}</Text>
                        )}

                        {result.attachments?.length > 0 ? (
                            <View style={styles.attachmentsWrap}>
                                {result.attachments.map((attachment, attachmentIndex) => (
                                    <TouchableOpacity
                                    ...
                                    >
                                        ...
                                        <Text style={styles.attachmentText} numberOfLines={1}>
                                            {getAttachmentTypeLabel(attachment)}: {attachment.originalName || attachment.filename}
                                        </Text>
                            ...
                            <Text style={styles.detailsEmpty}>No files attached.</Text>
                        )}
                    </View>
                ))}
            </View>
                        ...
                        <View>
                            ...
                            {inventoryActive.slice(0, invPage * PAGE_SIZE).map(item => (
                                <View key={item.id} style={styles.inventoryCard}>
                                    ...
                                    <TouchableOpacity style={styles.detailsToggle} onPress={() => toggleInventoryDetails(item.id)}>
                                        {expandedInventory[item.id] ? <ChevronUp size={16} color="#475569" /> : <ChevronDown size={16} color="#475569" />}
                                        <Text style={styles.detailsToggleText}>
                                            {expandedInventory[item.id] ? 'Hide full lab details' : 'Expand full lab details'}
                                        </Text>
                                    </TouchableOpacity>
                                    {expandedInventory[item.id] && renderLabDetails(item)}
                                </View>
                            ))}
...
const styles = StyleSheet.create({
    ...
    detailsToggle: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: 12, paddingVertical: 10, borderRadius: 8, backgroundColor: '#F8FAFC', borderWidth: 1, borderColor: '#E2E8F0' },
    detailsToggleText: { fontSize: 12, fontWeight: '700', color: '#475569' },
    labDetailsPanel: { marginTop: 12, borderTopWidth: 1, borderTopColor: '#E2E8F0', paddingTop: 12 },
    detailsTitle: { fontSize: 14, fontWeight: '800', color: '#1E293B', marginBottom: 6 },
    detailsLine: { fontSize: 12, color: '#475569', marginTop: 3 },
    detailsNote: { fontSize: 12, color: '#475569', marginTop: 5, fontStyle: 'italic' },
    detailsEmpty: { fontSize: 12, color: '#94A3B8', marginTop: 8 },
    resultBox: { backgroundColor: '#F8FAFC', borderWidth: 1, borderColor: '#E2E8F0', borderRadius: 8, padding: 10, marginTop: 10 },
    resultTitle: { fontSize: 13, fontWeight: '800', color: '#1E293B', marginBottom: 4 },
    attachmentsWrap: { gap: 8, marginTop: 10 },
    attachmentBtn: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#EFF6FF', borderWidth: 1, borderColor: '#BFDBFE', borderRadius: 8, padding: 10 },
    attachmentText: { flex: 1, color: '#1D4ED8', fontSize: 12, fontWeight: '700' },
