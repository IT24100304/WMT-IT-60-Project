...
import { FlaskConical, AlertTriangle, ShieldAlert, Clock, Search, History, Printer, Save, Paperclip, X } from 'lucide-react-native';
...
export default function LabDashboardScreen({ navigation }) {
    ...
    const [testForm, setTestForm] = useState({ hiv: false, hep: false, malaria: false, reason: '', files: [] });
    ...
    // --- Lab Test Logic ---
    const resetTestForm = () => setTestForm({ hiv: false, hep: false, malaria: false, reason: '', files: [] });
    ...
    const pickLabFiles = async () => {
        if (isPickingFiles || processingId) return;
        ...
        try {
            ...
            const pickedFiles = (result.assets || []).map((asset) => ({
                uri: asset.uri,
                name: asset.name || `lab-file-${Date.now()}`,
                type: asset.mimeType || 'application/octet-stream',
                size: asset.size
            }));

            setTestForm(prev => {
                const nextFiles = [...prev.files, ...pickedFiles].slice(0, 5);
                if (prev.files.length + pickedFiles.length > 5) {
                    Alert.alert('File Limit', 'You can attach up to 5 lab files per test.');
                }
                return { ...prev, files: nextFiles };
            });
        } catch (error) {
            Alert.alert('Upload Picker Error', 'Unable to open gallery or file picker.');
        } finally {
    ...
    const handleSubmitTestResult = async (bagId) => {
        if (isPickingFiles) {
            Alert.alert('Files Still Loading', 'Please wait until the selected files finish loading.');
            return;
        }
        ...
        try {
            ...
            const markerFound = [
                testForm.hiv ? 'HIV' : null,
                testForm.hep ? 'HEPATITIS' : null,
                testForm.malaria ? 'MALARIA' : null
            ].filter(Boolean);

            payload.append('hiv', testForm.hiv ? 'true' : 'false');
            payload.append('hep', testForm.hep ? 'true' : 'false');
            payload.append('malaria', testForm.malaria ? 'true' : 'false');
            payload.append('reason', trimmedReason || 'Routine Safe');
            payload.append('positiveDetails', JSON.stringify({
                markerFound,
                severity: markerFound.length > 1 ? 'HIGH' : 'MEDIUM',
                notes: trimmedReason
            }));

            testForm.files.forEach((file) => {
            ...
            await api.put(`/api/inventory/${bagId}/test`, payload, {
                                ...
                                <View key={bag.id} style={[
                                    ...
                                    {activeTestBagId === bag.id && (
                                        <View style={styles.testPanel}>
                                            ...
                                            <TouchableOpacity
                                                style={[styles.attachBtn, (isPickingFiles || processingId === bag.id) && styles.disabledBtn]}
                                                onPress={pickLabFiles}
                                                disabled={isPickingFiles || processingId === bag.id}
                                            >
                                                <Paperclip size={16} color="#2563EB" />
                                                <Text style={styles.attachBtnText}>
                                                    {isPickingFiles ? 'Loading selected files...' : `Attach gallery photos / PDF reports (${testForm.files.length}/5)`}
                                                </Text>
                                            </TouchableOpacity>

                                            {testForm.files.length > 0 && (
                                                <View style={styles.fileList}>
                                                    {testForm.files.map((file, index) => (
                                                        <View key={`${file.uri}-${index}`} style={styles.fileRow}>
                                                            <View style={{ flex: 1 }}>
                                                                <Text style={styles.fileName} numberOfLines={1}>{file.name}</Text>
                                                                {!!file.size && <Text style={styles.fileMeta}>{Math.ceil(file.size / 1024)} KB</Text>}
                                                            </View>
                                                            <TouchableOpacity onPress={() => removeLabFile(index)} style={styles.fileRemoveBtn}>
                                            ...
                                            <TouchableOpacity 
                                                style={[styles.saveBtn, (processingId === bag.id || isPickingFiles) && styles.saveBtnDisabled]} 
                                                disabled={processingId === bag.id || isPickingFiles}
                                                onPress={() => handleSubmitTestResult(bag.id)}
                                            >
                                                <Save size={16} color="#FFF" />
                                                <Text style={styles.saveBtnText}>
                                                    {processingId === bag.id ? 'Saving...' : (isPickingFiles ? 'Preparing files...' : 'Save Results')}
                                                </Text>
                                            </TouchableOpacity>
                                        </View>
                                    )}
                                            ...
                                            {historyLoadingBagId === bag.id ? (
                                                ...
                                                labResultsByBag[bag.id].map(row => (
                                                    <View key={row.id} style={styles.historyRow}>
                                                        ...
                                                        {row.attachments?.length ? <Text style={styles.historyNote}>Attachments: {row.attachments.length} file(s)</Text> : null}
                                                    </View>
                                                ))
                                            )}
                                        </View>
...
const styles = StyleSheet.create({
    ...
    reasonInput: { borderWidth: 1, borderColor: '#CBD5E1', borderRadius: 8, padding: 12, minHeight: 76, marginBottom: 12, backgroundColor: '#FFF', textAlignVertical: 'top' },
    attachBtn: { borderWidth: 1, borderColor: '#BFDBFE', backgroundColor: '#EFF6FF', borderRadius: 8, padding: 12, marginBottom: 10, flexDirection: 'row', alignItems: 'center', gap: 8 },
    attachBtnText: { color: '#1D4ED8', fontWeight: '700', fontSize: 12, flex: 1 },
    disabledBtn: { opacity: 0.55 },
    fileList: { marginBottom: 12, gap: 8 },
    fileRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F8FAFC', borderWidth: 1, borderColor: '#E2E8F0', borderRadius: 8, padding: 10 },
    fileName: { fontSize: 12, fontWeight: '700', color: '#1E293B' },
    fileMeta: { fontSize: 10, color: '#64748B', marginTop: 2 },
    fileRemoveBtn: { width: 28, height: 28, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: '#FEE2E2', marginLeft: 8 },
